...
#define INP_BUTTON_UP			(4) //	(0)
#define INP_BUTTON_DOWN			(6) //	(1)
#define INP_BUTTON_LEFT			(7) //	(2)
#define INP_BUTTON_RIGHT		(5) //  (3)
#define INP_BUTTON_HARDB		(14) //	(4)
#define INP_BUTTON_HARDA		(13) //	(5)
#define INP_BUTTON_HARDAB		(15) //	(6)
#define INP_BUTTON_START		(3) //	(7)
#define INP_BUTTON_SELECT		(0) //	(8)
...
int EMU_MainLoop(void)
{
    ...
    if(fb.enabled) 
	{
		...
		//memcpy(BaseAddress, vidram, 160*144*2);	updateScreen();
        disp_lcd(vidram);  // ‰æ–Ê‚ÌXV
...
/*
		...
		if(tval.tv_usec < lim_time) { // we are too fast
			...
			if(!turbomode) { simpleWait(thissec, lim_time);	}
...
//#define GB_HANDLEKEY(inkey,outkey) if(newkeys & inkey) { hw.pad |= PAD_##outkey; printf("[%x %x]\n",inkey, PAD_##outkey);}
