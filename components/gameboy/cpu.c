...
{
	cpu.lcdc -= cnt * 2; // LCDÇÃèàóùÇä‘à¯Ç≠ 
	if (cpu.lcdc <= 0) {
        int ct0=(int)clock();
        lcdc_trans();
        cput.t3 = cput.t3 + (int)clock() - ct0;
    }
}
...
//	if (debug_trace) debug_disassemble(PC, 1);
	op = FETCH;
    //printf("[%x:%x]", PC-1, op); fflush(stdout);
	...
	//clen <<= 1;
	//div_advance(clen);
	timer_advance(clen); //Timer_interrupt
	//clen >>= cpu.speed;
	lcdc_advance(clen);
    //cpu.snd += clen; // sound_advance(clen);
	i -= clen;
	if (i > 0) goto next;
    
    cput.t4 = cput.t4 + XTHAL_GET_CCOUNT()- ct0;
	return cycles-i;
}
